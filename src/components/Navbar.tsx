...
import { BillCategory, BillType } from '../types';
import { 
  FileText, 
  BookUser, 
  Layers, 
...
export const Navbar: React.FC<Props> = ({
  ...
  return (
    <header className="bg-slate-900/95 backdrop-blur border-b border-slate-800 sticky top-0 z-40 px-4 py-2.5">
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="font-extrabold text-lg text-white tracking-tight">BillCrafter</span>
              <span className="text-[10px] uppercase font-black px-1.5 py-0.5 rounded bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">
                PRO
              </span>
            </div>
            <p className="text-[11px] text-slate-400">Universal Bill Generator & Customizer</p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-2">
          <div className="relative">
            <select
              ...
              className="appearance-none bg-slate-800 border border-slate-700 hover:border-indigo-500 text-white font-semibold text-xs rounded-xl pl-3 pr-8 py-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 cursor-pointer shadow-sm"
            >
          ...
          <button
            ...
            className="px-3 py-2 bg-gradient-to-r from-amber-600 to-orange-600 hover:from-amber-500 hover:to-orange-500 text-white font-bold text-xs rounded-xl flex items-center gap-1.5 shadow-lg shadow-amber-900/30 transition"
          >
            <Layers className="w-3.5 h-3.5" /> Multi-Bill A4 Stacker
          </button>
        </div>

        {/* Action Tools: Address Book, Randomize, PDF, Image, Print */}
        <div className="flex items-center space-x-2">
          <button
            onClick={onOpenAddressBook}
            className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 font-semibold text-xs rounded-xl flex items-center gap-1.5 border border-slate-700 hover:border-slate-600 transition relative"
          >
          ...
          <button
            ...
            className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 font-semibold text-xs rounded-xl flex items-center gap-1.5 border border-slate-700 transition"
          >
          ...
          <button
            onClick={onExportPdf}
            className="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl flex items-center gap-1.5 shadow-lg shadow-emerald-900/30 transition"
          >
      ...
      <div className="max-w-[1600px] mx-auto mt-2 pt-2 border-t border-slate-800/80 flex items-center space-x-1.5 overflow-x-auto pb-1 text-xs">
        {CATEGORIES.map((cat) => (
          <button
            key={cat.id}
